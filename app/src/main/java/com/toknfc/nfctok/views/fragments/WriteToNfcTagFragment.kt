...
import com.toknfc.nfctok.NfcUtilities.Companion.writeMessageToTag
...
import com.toknfc.nfctok.extensions.toNdefMessage
...
class WriteToNfcTagFragment : CoreFragment(), WriteToNfcTagFragmentPresenter.View {
  ...
  override fun createNfcMessage(): Boolean {
    val ndefMessage = fetchPayload().toNdefMessage()
    nfcIntent.apply {
      val tag = getParcelableExtra<Tag>(NfcAdapter.EXTRA_TAG)
      try {
        return writeMessageToTag(ndefMessage, tag, context) {e -> handleError(e)}
      } catch (exc: Exception) {
        handleError(exc)
      }
    }
